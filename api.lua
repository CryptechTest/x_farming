...
function x_farming.grow_large_cactus(pos)
    ...
    if (math.random(1,2) == 1) then
        path = core.get_modpath('x_farming') ..
    ...
    for _, v in pairs(core.registered_decorations) do
        ...
        if _decoration then
            for _, d in pairs(_decoration) do
                local deco_node = core.registered_nodes[d]
...
function x_farming.register_feast(name, def)
    ...
    local _def = {
        ...
        on_use = function(itemstack, user, pointed_thing)
            ...
            return core.item_eat(hunger_amount, "x_farming:bowl 4")(itemstack, user, pointed_thing)
        end,
    }
    ...
    for i = 1, def.steps do
        ...
        local craftitem_def = {
            description = def.short_description .. ' ' .. S('Bowl') .. '\n' .. S('Compost chance') .. ': 100%\n'
                .. core.colorize(x_farming.colors.brown, S('Hunger') .. ': ' .. tostring(def.hunger_amount and def.hunger_amount / 4)),
